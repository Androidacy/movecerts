...
detect_ext_data() {
  export EXT_DATA='/sdcard/Documents/movecerts'
...
setup_logger() {
  ...
  export LOGFILE
  {
    echo "[INFO] Module: Move Certificates $(grep 'version=' $MODPATH/module.prop | cut -d"=" -f2)"
...
cleanup() {
  ...
  ui_print "**************************************"
  ui_print " "
  logUploader "$EXT_DATA/logs/install-debug.log"
}
...
if test $API -lt 24; then
  abort "! Your system API of $API is less than the minimum api of 24 (Nougat)! Aborting!"
fi
